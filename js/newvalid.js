...
{
	let namesInp = ['date', 'dateBuy'];
	namesInp.forEach(element => {
		...
		//obj.onchange = valid(element, nameForm);
		obj.onchange = function()
		{	
			let birth = document.getElementById(element);
			let tmpres = xcheck(nameForm, element, birth.value);
		
			if(tmpres==1)
			{
				birth.parentElement.classList.remove('errorInput');
				birth.parentElement.querySelector('div').classList.add('hidden');
				birth.parentElement.querySelector('span').classList.add('hiddenSpan');
				birth.parentElement.classList.add('filled');
				xsettings[nameForm][element]['valid'] = true;
				checkButton(nameForm);
			}
			else if(tmpres==0)
			{
				birth.parentElement.classList.add('errorInput');
				birth.parentElement.querySelector('div').classList.remove('hidden');
				birth.parentElement.querySelector('span').classList.remove('hiddenSpan');
				birth.parentElement.classList.remove('filled');
				xsettings[nameForm][element]['valid'] = false;
				checkButton(nameForm);
			}
		}
	});
